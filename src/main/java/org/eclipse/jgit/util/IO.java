...
public class IO {
   ...
   * @throws java.io.IOException the file exists, but its contents cannot be read.
   */
   ...
   *
   * @param path location of the file to read.
   * @param limit maximum number of bytes to read, if the file is larger than only the first limit
   *     number of bytes are returned
   * @return complete contents of the requested local file. If the contents exceeds the limit, then
   *     only the limit is returned.
   * @throws java.io.FileNotFoundException the file does not exist.
   * @throws java.io.IOException the file exists, but its contents cannot be read.
   */
   ...
   *
   * @param path location of the file to read.
   * @param max maximum number of bytes to read, if the file is larger than this limit an
   ...
   *
   * <p>Note: The stream is read to its end and is not usable after calling this method. The caller
   * is responsible for closing the stream.
   *
   * @param in input stream to be read.
   * @param sizeHint a hint on the approximate number of bytes contained in the stream, used to
   *     allocate temporary buffers more efficiently
   * @return complete contents of the input stream. The ByteBuffer always has a writable backing
   *     array, with {@code position() == 0} and {@code limit()} equal to the actual length read.
   *     Callers may rely on obtaining the underlying array for efficient data access. If {@code
   *     sizeHint} was too large, the array may be over-allocated, resulting in {@code limit() <
   *     array().length}.
  ...
  public static ByteBuffer readWholeStream(InputStream in, int sizeHint) throws IOException {
    return ByteBuffer.wrap(readAllBytes(in));
  }
  ...
  public static void readFully(final InputStream fd, final byte[] dst, int off, int len)
      throws IOException {
    int read = readNBytes(fd, dst, off, len);
    if (read != len) throw new EOFException(JGitText.get().shortReadOfBlock);
  }
   ...
   * @throws java.io.IOException there was an error reading from the channel.
   */
  ...
  public static int readFully(InputStream fd, byte[] dst, int off) throws IOException {
    return readNBytes(fd, dst, off, dst.length - off);
  }

  /**
   * Skip an entire region of an input stream.
   *
   * <p>The input stream's position is moved forward by the number of requested bytes, discarding
   * them from the input. This method does not return until the exact number of bytes requested has
   * been skipped.
   *
   * @param fd the stream to skip bytes from.
   * @param toSkip total number of bytes to be discarded. Must be &gt;= 0.
   * @throws EOFException the stream ended before the requested number of bytes were skipped.
   * @throws java.io.IOException there was an error reading from the stream.
   */
  public static void skipFully(InputStream fd, long toSkip) throws IOException {
    ...
    while (toSkip > 0) {
      ...
      if (r <= 0) throw new EOFException(JGitText.get().shortSkipOfBlock);
      toSkip -= r;
    }
  }
